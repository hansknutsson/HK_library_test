function [Vwgts, ErrSPH, A, W] = Optimize_SPH_filters(vecs,varargin)
%function [Vwgts, ErrSPH, A, W] = Optimize_SPH_filters(vecs,varargin)
%
...
% --------------------------
% vecs:  [3,N] ([1,1], [])  - Input vector array (or number of precomputed vectors, [] => default = 21)
%                                                (possible choises are 16,21,30,46,61,102,120,200,300)
...
% rcosexp  = 3;    Real      - Sets cos^exp type weight function factor in optimization.
...
% rcosexpS = 1;    Real      - Sets cos^exp type test SPH spectrum factor.
...
% plotlev  = 0;    Integer   - 0 => no plots,  1 => a few plots,  2 = many plots.
% rendSPH  = 0;    [0,1]     - 1 => render color coded SPH weights on spheres. 
...
if nargin<1  | numel(vecs)==0;
    vecs = 21;                % chose from precomputed oneshell distributions below
end

if numel(vecs) == 1;
    switch vecs
      case 16;  dName = 'Generated_Distributions/X_016_Qdistr_03633__1.00_2.00_8.00_1.00_0.00_J0_S1_1.00_1.50_0.03_0.00.mat';
      case 21;  dName = 'Generated_Distributions/X_021_Qdistr_03629__1.00_2.00_8.00_1.00_0.00_J0_S1_1.00_1.50_0.03_0.00.mat'; % (used for ISMRM 2019)
      case 30;  dName = 'Generated_Distributions/X_030_Qdistr_03631__1.00_2.00_8.00_1.00_0.00_J0_S1_1.00_1.50_0.03_0.00.mat';
      case 46;  dName = 'Generated_Distributions/X_46_SHELL.mat';   % (very good)
      case 61;  dName = 'Generated_Distributions/X_061_Qdistr_03632__1.00_2.00_8.00_1.00_0.00_J0_S1_1.00_1.50_0.03_0.00.mat';
      case 102; dName = 'Generated_Distributions/X_102_Qdistr_03634__1.00_2.00_8.00_1.00_0.00_J0_S1_1.00_1.50_0.03_0.00.mat';
      case 120; dName = 'Generated_Distributions/X_120_SHELL.mat';  % (very good)      
      case 200; dName = 'Generated_Distributions/X_200_oneSHELL_2.00_2.00_2.00_1.00_1.00_J0_S1.mat'; % (very good)
      case 300; dName = 'Generated_Distributions/X_300_Qdistr_03635__1.00_2.00_8.00_1.00_0.00_J0_S1_1.00_1.50_0.03_0.00.mat';
...
maxdeg  = 2*round(0.75*(sqrt(1+3*Nvec*8/9)-1));  % aprox 3 X above SPH 'Nyqusist'  (must be above SPH Nyq)
Nlost   = sum(cert <= 0);
Nvecok  = Nvec - Nlost;
...
rendSPH  = 0;  
fignr1   = 5000; 
saveCode = 0;             % 1 => The code that is beeing executed will be saved.

for nvin = 1:2:numel(varargin)
    switch varargin{nvin}
      case 'Vwgts'
        Vwgts0     = varargin{nvin+1}; optimize   = 0;
      case 'rcosexp'
        rcosexp    = varargin{nvin+1};
      case 'wrW'
        wrW        = varargin{nvin+1};
      case 'd0'
        d0         = varargin{nvin+1};
      case 'rcosexpS'
        rcosexpS   = varargin{nvin+1};
      case 'wrS'
        wrS        = varargin{nvin+1};
      case 'ppars'
        ppars      = varargin{nvin+1};
      case 'peval'
        peval      = varargin{nvin+1};
      case 'plotlev'
        plotlev    = varargin{nvin+1};
      case 'rendSPH'
        rendSPH    = varargin{nvin+1};
      case 'fignr1'
        fignr1     = varargin{nvin+1};
      case 'saveCode'
        saveCode   = varargin{nvin+1};
      otherwise
        help SPH3_optimize_2nd_degree_filter;
...
maxdegp = maxdeg;%2*ceil(0.75*(sqrt(1+1.5*Nvec*8/9)-1)) + 2;   % more than 1.5 X above SPH 'Nyqusist' 

cosexp0 = ((maxdeg+eps)/(degref+eps))^2 - 1;
%cosexp0old = (maxdeg + 1)*(maxdeg + 2)/2/sqrt(Nvec*Nvecok)
...
maxpnd  = maxdegp/2 + 1;
maxpnd2 = ceil(maxdegp/6) + 1;

if ppars
    if optimize   % no input weights are given
        fprintf(1,'\n  Optimizing SPH fiters')
        fprintf(1,'\n  ----------------------------------------------------------')
        fprintf(1,'\n  Nvec  = %3i      maxdeg =%3i      degref =%5.1f    d0 =%2i',Nvec,maxdeg,degref,d0)
        fprintf(1,'\n  Nlost = %3i      cosexp =%5.2f       wrW =%5.2f \n\n',Nlost,cosexp,wrW)
    else
        fprintf(1,'\n  Evaluating SPH fiters')
        fprintf(1,'\n  ----------------------------------------------------------')
        fprintf(1,'\n  Nvec  = %3i      maxdeg  =%3i',Nvec,maxdeg)
...
for degree = degrees      % l in standard SPH notaion
    ...
    SPHd0 = [pall(1,:); reshape(permute(SPHd0,[3 1 2]),2*degree,Nvec)];
...
if optimize  % no input weights are given
    ...
    %B_dual   = (A + 1e-8*max(diag(A))*eye(size(A)))^(-1)*(B*W2);   % weight vector set
    %B_dual   = inv(A + 1e-8*max(diag(A))*eye(size(A)))*(B*W2);   % weight vector set
    %B_dual   = pinv(A)*(B*W2);                                  % weight vector set
else
    sndeg = 0;
    for degree = degrees
        ndeg   = degree/2+1;
...
%Err_SPH__ = Err_SPH(end,:)
%wErr_SPH__ = wErr_SPH(end,:)
...
if peval > 0
    ...
    fprintf('\n  ----------------------------------------------------')
    fprintf('\n  Using opt weight         %7.5f   %7.5f   %7.5f', wErr_SPH(end,1), wErr_SPH(end,2), wErr_SPH(end,3))
    fprintf('\n  Using signal weight      %7.5f   %7.5f   %7.5f \n\n', Err_SPH(end,1), Err_SPH(end,2), Err_SPH(end,3))
end
...
if plotlev > 0
    ...
    title('Total error vs signal max SPH degree')
    hold off
...
if plotlev > 1
    ...
    image(RGB)
    %gopimage(abs(BtBd0).^0.25.*exp(i*ccol*DM))
    axis image

    %szErrSPH_1 = size(Err_SPH_1)
    %szErr_orig = size(Err_orig)
    ...
    %surf(ppx,ppy,abs(pplBB).^magexp,0.1+abs(pplBB).^colexp,'ambientstrength',ast,'diffusestrength',dst,'specularstrength',sst)
    shading('interp')
    xlim(xylim989)
    ylim(xylim989)
    zlim([0 max989])
    ...
    %surf(ppx,ppy,abs(pplBtBd).^magexp,abs(pplBtBd).^colexp,'ambientstrength',ast,'diffusestrength',dst,'specularstrength',sst)
    shading('interp')
    xlim(xylim989)
    ylim(xylim989)
    zlim([0 max989])
    ...
    RGB = Gop_RGB_func(exp(i*(coff+calf*(gim+gim0).^colexp)),[],csat);
    surf(ppx,ppy,gim.^magexp,RGB,'ambientstrength',ast,'diffusestrength',dst,'specularstrength',sst)
    %surf(ppx,ppy,abs(pplwBB).^magexp,abs(pplwBB).^colexp,'ambientstrength',ast,'diffusestrength',dst,'specularstrength',sst)
    shading('interp')
    xlim(xylim989)
    ylim(xylim989)
    zlim([0 max989])
    ...
    RGB = Gop_RGB_func(exp(i*(coff+calf*(gim+gim0).^colexp)),[],csat);
    surf(ppx,ppy,gim.^magexp,RGB,'ambientstrength',ast,'diffusestrength',dst,'specularstrength',sst)
    %surf(ppx,ppy,abs(pplwBtBd).^magexp,abs(pplwBtBd).^colexp,'ambientstrength',ast,'diffusestrength',dst,'specularstrength',sst)
    shading('interp')
    xlim(xylim989)
    ylim(xylim989)
    zlim([0 max989])
    ...
    xlabel('SPH filter number')
    %grid off
    %set(h,'color','none','Zcolor','none','ztick',[])
    %set(h,'Zcolor','none')
    %h.ZAxis.Visible = 'off';
    %axis off
    %set(h,'dataaspectratio',[25 25 1])
    %set(h,'cameraviewangle',6)
    %set(h,'PlotBoxAspectRatio',[2 2 2])
    %set(h,'Box','off')
    set(h,'Clipping','off')
    %get(h)
    light('position',lip1)
...
if rendSPH
    ...
    alfa2   = 0.05;
    bsz     = 0.017;
    rballsz = min(16*(240/Nvec)^0.25,29);
    shexp2  = 2;
    sballs  = 1;
    lincol  = 0;
    metric  = [8 1 1 1 2];
    ...
    for ndeg = 1:Ndeg
        ...
        SPHnrm = (SPHnrm + 1)/2;           
        %nsp    = 4*(Ndeg-1)*(ndeg-1) + 2*(Ndeg-1)+1 - ndeg;  % symmetric pyramid
        nsp    = (4*(Ndeg-1)+1)*(ndeg-1);                    % left to right for each degree
        for ord = 1:4*(ndeg-1)+1;
            nB  = nB + 1;
            nsp = nsp+1;
            subplotHK(Ndeg,4*(Ndeg-1)+1,nsp)
...
%save resutls, and the code generating the results (for provenance)
%fName = [mfilename '.m'];
%gradient_waveform.mfile = fileread(fName);
%save gradient_waveforms gradient_waveform 
